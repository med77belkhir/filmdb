    ...
    if (user?.id) {
      if (!isAddedToList) {
        ...
        try {
          const { data, error } = await supabase
            .from("watchlist")
            .insert([{ movie_id: props.data.imdbID, user_id: user.id }]);
        ...
        try {
          ...
          if (error) {
            console.error(
              "Error removing movie from watch list:",
              error.message
            );
          } else {
    ...
    <div className="row">
      <div className="col-lg-8 col-md-8 col-sm-12">
        <div className="row">
          ...
          >
            {genres.map((item) => (
              <span style={styleElement} key={item}>
                {" "}
                {item}
              </span>
            ))}
          ...
          <div style={{ color: "white", textDecoration: "none" }}>
            ...
            <span style={{ color: "blue" }}> {props.data.Director} </span>
          </div>
          <hr style={{ color: "white", textDecoration: "none" }}></hr>
          <div style={{ color: "white", textDecoration: "none" }}>
            <span style={{ textTransform: "uppercase", fontSize: "1em" }}>
            ...
            <span style={{ color: "blue" }}> {props.data.Writer} </span>
          </div>
          <hr style={{ color: "white", textDecoration: "none" }}></hr>
          <div style={{ color: "white", textDecoration: "none" }}>
            <span style={{ textTransform: "uppercase", fontSize: "1.2em" }}>
              Stars{" "}
            </span>
            <span style={{ color: "blue" }}> {props.data.Actors} </span>
          </div>
          <hr style={{ color: "white", textDecoration: "none" }}></hr>
          <div style={{ color: "white", textDecoration: "none" }}>
          ...
          <div>
            <a
              ...
              style={{ color: "yellow", textDecoration: "overline " }}
            >
              See your reviews
            </a>{" "}
            <br />
            {showReviews &&
              (whatchReview.length > 0 ? (
                whatchReview.map((review, index) => {
                  return (
                    <div key={`${review.movie_id}-${index}`}>
                      <MovieCard
                        movie={review}
                        review={review.review}
                        create={review.created_at}
                      />
                    </div>
                  );
                })
      ...
      >
        ...
        >
          <FontAwesomeIcon icon={isAddedToList ? faMinus : faPlus} />{" "}
          {isAddedToList ? "Remove from watch list" : "Add to watch list"}
        </button>

        <button
          style={{ ...buttonReviewStyle, marginTop: "20px" }}
        ...
        {showReview && (
          <div style={{ marginTop: "20px", color: "white" }}>
            ...
            />
            <button
              style={{ ...buttonReviewStyle, marginTop: "10px" }}
